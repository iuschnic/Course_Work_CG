...
public:
    ...
    {
        double delta_a = 2.0 * M_PI / SEGMENTS_PARALLEL;
        ...
        for (int i = 1; i < SEGMENTS_PARALLEL; ++i)
        {
            beta = 0;
            cur_points.clear();
            ...
            else if (i == SEGMENTS_PARALLEL - 1)
            {
                ...
                size++;
                //Верхний полюс уже занесен в массив, поэтому учитываем смещение
                //Заносим в массив граней (грань = массив точек, принадлежащих грани)
                for (int t = 0; t < cur_points.size() - 1; t++)
                {
                    std::vector<int> face = {size - 1, t + size, t + size + 1};
                    _faces.push_back(face);
                }
                //Занесение грани между первой и последней точкой параллели
                std::vector<int> face = {size - 1, size, size + cur_points.size() - 1};
                _faces.push_back(face);
            }
            ...
            else
            {
                ...
                for (int t = 0; t < cur_points.size() - 1; t++)
                {
                    std::vector<int> face = {size - 1 - t, size - t, size - t + cur_points.size(), size - 1 - t + cur_points.size()};
                    _faces.push_back(face);
                }
                std::vector<int> face = {size - 1, size + cur_points.size() - 2, size + 2 * cur_points.size() - 2, size - 1 + cur_points.size()};
                _faces.push_back(face);
            }
    ...
    bool check_intersection(const Sphere &sphere)
    {
        ...
        double dist = ((c.get_x() - get_x())**2 + (c.get_y() - get_y())**2 + (c.get_z() - get_z())**2)**0.5;
        if (dist > sum_r)
            return false;
        return true;
    }
